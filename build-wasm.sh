...
# Merge and copy model (required for ONNX Runtime Web - must be self-contained)
if [ -f assets/encoder.onnx ]; then
    ...
    # Ensure no .onnx.data file exists in www/assets
    rm -f www/assets/encoder.onnx.data
else
    echo "Warning: encoder.onnx not found"
fi
